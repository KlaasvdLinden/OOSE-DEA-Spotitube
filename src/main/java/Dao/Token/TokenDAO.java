...
public class TokenDAO extends DAO {
    ...
    public ResponseLogin updateToken(int userID, String username) {
        ...
        responseLogin = generateAndSaveToken(userID, username, UPDATE_TOKEN_QUERY);
        return responseLogin;
    }


    private void saveToken(User user, String query) {
    ...
    private ResponseLogin generateAndSaveToken(int userID, String username, String query) {
        User user = new User();
        user.setToken(generateToken(TOKEN_LENGTH));
        user.setId(userID);
        saveToken(user, query);
